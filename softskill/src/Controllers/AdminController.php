...
class AdminController
{
    ...
    public function deleteRecruiter()
    {
        if (isset($_GET['id'])) {
            ...
            $stmt = $conn->prepare("DELETE FROM users WHERE id = :id AND role = 'recruiter'");
    ...
    public function settings()
    {
        $db = new Database();
        $conn = $db->getConnection();
        $stmt = $conn->prepare("SELECT setting_value FROM settings WHERE setting_key = 'openai_api_key' LIMIT 1");
        $stmt->execute();
    ...
    public function saveSettings()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Upsert
            $stmt = $conn->prepare("REPLACE INTO settings (setting_key, setting_value) VALUES ('openai_api_key', :key)");
    ...
    public function questions()
    {
        ...
        $stmt = $conn->prepare("SELECT * FROM questions WHERE test_type = :type");
    ...
    public function saveQuestion()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $db = new Database();
            $conn = $db->getConnection();
            $stmt = $conn->prepare("INSERT INTO questions (test_type, question_text, dimension) VALUES (:type, :text, :dim)");
    ...
    public function updateQuestion()
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $db = new Database();
            $conn = $db->getConnection();
            $stmt = $conn->prepare("UPDATE questions SET question_text = :text, dimension = :dim WHERE id = :id");
            $stmt->bindParam(':text', $_POST['text']);
            $stmt->bindParam(':dim', $_POST['dimension']);
