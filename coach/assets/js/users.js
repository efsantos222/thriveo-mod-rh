...
function skeletonUsersGrid() {
    return Array(6).fill(0).map(() => `
        <div class="user-card" style="pointer-events:none;">
            <div class="user-header">
                <div class="skeleton skeleton--avatar" style="width:3rem;height:3rem;border-radius:50%;flex-shrink:0;"></div>
                <div style="flex:1;display:flex;flex-direction:column;gap:.5rem;">
                    <div class="skeleton skeleton--title" style="width:60%;"></div>
                    <div class="skeleton skeleton--text" style="width:80%;"></div>
                    <div class="skeleton skeleton--text" style="width:40%;height:.875rem;border-radius:999px;"></div>
                </div>
            </div>
        </div>
    `).join('');
...
function loadUsers() {
    ...
    grid.innerHTML = skeletonUsersGrid();
    ...
    fetch('../api/users/update.php', {
        ...
        .then(result => {
            if (result.success) {
                closeNewUserModal();
                loadUsers();
                if (window.Toast) Toast.success('Usuário atualizado com sucesso!');
            } else {
                if (window.Toast) Toast.error(result.message || 'Erro ao atualizar usuário.');
            }
        });
}
    ...
    fetch('../api/users/create.php', {
        ...
        .then(result => {
            if (result.success) {
                closeNewUserModal();
                loadUsers();
                if (window.Toast) Toast.success('Usuário criado com sucesso!');
            } else {
                if (window.Toast) Toast.error(result.message || 'Erro ao criar usuário.');
            }
        })
        .catch(error => {
            console.error('Error:', error);
            if (window.Toast) Toast.error('Erro ao criar usuário.');
        });
}
...
function deleteUser(id) {
    ...
    fetch('../api/users/delete.php', { // Assuming you have/will have this
        ...
        .then(data => {
            if (data.success) {
                loadUsers();
                if (window.Toast) Toast.success('Usuário excluído.');
            } else {
                if (window.Toast) Toast.error(data.message || 'Erro ao excluir usuário.');
            }
        });
}
