...
$candidatesStmt = $pdo->prepare("SELECT id, name, linkedin_pdf FROM entrev_candidates WHERE company_id = ? ORDER BY name ASC");
$candidatesStmt->execute([$company_id]);
$allCandidates = $candidatesStmt->fetchAll();

// Verify Job ownership
$stmt = $pdo->prepare("SELECT * FROM entrev_jobs WHERE id = ? AND company_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    // Get API Key
    $apiKeyStmt = $pdo->query("SELECT setting_value FROM entrev_settings WHERE setting_key = 'openai_api_key'");
    ...
    if (!$apiKey) {
        ...
        if ($_POST['action'] === 'generate_ai') {
            ...
            $stmt = $pdo->prepare("SELECT name, linkedin_pdf FROM entrev_candidates WHERE id = ? AND company_id = ?");
        ...
        if (isset($data['messages']) && !$message) {
            ...
            if (isset($result['choices'][0]['message']['content'])) {
                ...
                // Save valid script
                $save = $pdo->prepare("INSERT INTO entrev_interview_scripts (job_id, content) VALUES (?, ?)");
...
$scripts = $pdo->prepare("SELECT * FROM entrev_interview_scripts WHERE job_id = ? ORDER BY created_at DESC");
