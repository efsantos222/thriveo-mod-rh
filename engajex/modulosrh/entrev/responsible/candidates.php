    ...
    if (isset($_POST['action'])) {
        if ($_POST['action'] === 'create') {
            ...
            if (!$message) { // Proceed if no upload error
                $stmt = $pdo->prepare("INSERT INTO entrev_candidates (company_id, name, email, linkedin_pdf) VALUES (?, ?, ?, ?)");
            ...
            $id = $_POST['id'];
            $check = $pdo->prepare("SELECT id, linkedin_pdf FROM entrev_candidates WHERE id = ? AND company_id = ?");
            ...
            if ($candidate) {
                ...
                $stmt = $pdo->prepare("DELETE FROM entrev_candidates WHERE id = ?");
                $stmt->execute([$id]);
...
$candidates = $pdo->prepare("SELECT * FROM entrev_candidates WHERE company_id = ? ORDER BY created_at DESC");
