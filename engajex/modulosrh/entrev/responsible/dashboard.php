...
// Stats
$stmt = $pdo->prepare("SELECT COUNT(*) FROM entrev_jobs WHERE company_id = ?");
$stmt->execute([$company_id]);
$totalJobs = $stmt->fetchColumn();

$stmt = $pdo->prepare("SELECT COUNT(*) FROM entrev_candidates WHERE company_id = ?");
$stmt->execute([$company_id]);
$totalCandidates = $stmt->fetchColumn();

// Recent Jobs
$stmt = $pdo->prepare("SELECT * FROM entrev_jobs WHERE company_id = ? ORDER BY created_at DESC LIMIT 5");
$stmt->execute([$company_id]);
