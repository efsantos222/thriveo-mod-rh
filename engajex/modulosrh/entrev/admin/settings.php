...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($apiKey) {
        $stmt = $pdo->prepare("UPDATE entrev_settings SET setting_value = ? WHERE setting_key = 'openai_api_key'");
        $stmt->execute([$apiKey]);
        // Also check if it existed, if not insert (edge case but good to have)
        if ($stmt->rowCount() == 0) {
            // Check if row actually exists
            $check = $pdo->query("SELECT count(*) FROM entrev_settings WHERE setting_key = 'openai_api_key'")->fetchColumn();
            if ($check == 0) {
                $stmt = $pdo->prepare("INSERT INTO entrev_settings (setting_key, setting_value) VALUES ('openai_api_key', ?)");
                $stmt->execute([$apiKey]);
            }
        }
...
$stmt = $pdo->query("SELECT setting_value FROM entrev_settings WHERE setting_key = 'openai_api_key'");
