...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($_POST['nome']) || empty($_POST['email']) || empty($_POST['cargo']) || 
        ...
        header('Location: register_candidate_mbti.php?error=missing_fields');
        exit;
    }
    ...
    if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header('Location: register_candidate_mbti.php?error=invalid_email');
        exit;
    }

    // Verificar se a senha tem pelo menos 6 caracteres
    if (strlen($senha) < 6) {
        header('Location: register_candidate_mbti.php?error=invalid_password');
        exit;
    }
    ...
    if ($email_exists) {
        header('Location: register_candidate_mbti.php?error=email_exists');
        exit;
    }
    ...
    if (!file_exists($candidatos_file)) {
        error_log("Criando arquivo candidatos_mbti.csv");
        ...
        if ($fp !== false) {
            ...
            error_log("Erro ao criar arquivo candidatos_mbti.csv");
            header('Location: register_candidate_mbti.php?error=file_error');
            exit;
        }
    }
    ...
    if ($fp !== false) {
        ...
        error_log("Salvando dados do candidato MBTI:");
        ...
        if (fputcsv($fp, $dados)) {
            ...
            fclose($fp);
            header('Location: register_candidate_mbti.php?success=1');
            exit;
        } else {
...
header('Location: register_candidate_mbti.php');
exit;
