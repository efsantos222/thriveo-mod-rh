...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['action'])) {
        if ($_POST['action'] === 'create') {
            ...
            $stmt = $pdo->prepare("INSERT INTO jobs (company_id, title, description, specifications) VALUES (?, ?, ?, ?)");
            $stmt->execute([$company_id, $title, $description, $specifications]);
            $message = "Vaga criada com sucesso.";
        } elseif ($_POST['action'] === 'delete') {
            $id = $_POST['id'];
            // Verify ownership
            $check = $pdo->prepare("SELECT id FROM jobs WHERE id = ? AND company_id = ?");
...
$jobs = $pdo->prepare("SELECT * FROM jobs WHERE company_id = ? ORDER BY created_at DESC");
