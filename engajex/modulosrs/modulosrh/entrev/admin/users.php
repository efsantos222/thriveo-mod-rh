        ...
        if ($_POST['action'] === 'create') {
            ...
            if ($name && $email && $password && $company_id) {
                ...
                if ($stmt->fetchColumn() > 0) {
                    ...
                    $stmt = $pdo->prepare("INSERT INTO users (name, email, password, role, company_id) VALUES (?, ?, ?, 'responsible', ?)");
                    try {
...
$users = $pdo->query("SELECT u.*, c.name as company_name FROM users u LEFT JOIN companies c ON u.company_id = c.id WHERE u.role = 'responsible' ORDER BY u.created_at DESC")->fetchAll();
