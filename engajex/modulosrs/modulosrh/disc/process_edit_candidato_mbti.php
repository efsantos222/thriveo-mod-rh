...
if (empty($_POST['nome']) || empty($_POST['email']) || empty($_POST['email_original']) || empty($_POST['cargo']) || empty($_POST['selecionador'])) {
    header('Location: superadmin_panel.php?error=missing_fields#candidatos-mbti');
    exit;
}
...
if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
    header('Location: superadmin_panel.php?error=invalid_email#candidatos-mbti');
    exit;
}

// Validar senha se fornecida
if (!empty($senha) && strlen($senha) < 6) {
    header('Location: superadmin_panel.php?error=invalid_password#candidatos-mbti');
    exit;
}
        ...
        if ($fp !== false) {
            while (($data = fgetcsv($fp)) !== FALSE) {
                if ($data[4] === $email) { // Coluna do e-mail
                    fclose($fp);
                    header('Location: superadmin_panel.php?error=email_exists#candidatos-mbti');
                    exit;
                }
            }
            fclose($fp);
...
if ($fp !== false) {
    ...
    if ($email !== $email_original) {
        $old_file = 'resultados/' . str_replace(['@', '.'], '_', $email_original) . '_avaliacao_mbti.csv';
        $new_file = 'resultados/' . str_replace(['@', '.'], '_', $email) . '_avaliacao_mbti.csv';
        if (file_exists($old_file)) {
            rename($old_file, $new_file);
        }
    }
    
    header('Location: superadmin_panel.php?success=edit_candidato#candidatos-mbti');
    exit;
} else {
    header('Location: superadmin_panel.php?error=file_error#candidatos-mbti');
    exit;
}
