...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    header('Location: view_candidates_disc.php');
    exit;
}
...
if (empty($email) || empty($nova_senha)) {
    header('Location: view_candidates_disc.php?error=empty_fields');
    exit;
}
...
if (!file_exists($candidatos_file)) {
    header('Location: view_candidates_disc.php?error=file_not_found');
    exit;
}
...
if ($fp_read === false || $fp_write === false) {
    header('Location: view_candidates_disc.php?error=file_error');
    exit;
}
...
header('Location: view_candidates_disc.php?success=password_updated');
exit;
